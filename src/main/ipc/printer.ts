...
async function sendGdiTextToWindowsPrinter(printerName: string, text: string): Promise<void> {
  ...
  const ps = [
    ...
    '  $clipW = [float]$e.Graphics.VisibleClipBounds.Width',
    '',
    '  # Mede a largura real de uma linha de 48 chars para dimensionar o bitmap corretamente',
    '  $tmpBmp = New-Object System.Drawing.Bitmap(1,1)',
    '  $tmpG   = [System.Drawing.Graphics]::FromImage($tmpBmp)',
    '  $ruler  = [string]::new([char]"=", 48)',
    '  $measW  = [int]($tmpG.MeasureString($ruler, $font).Width + 4)',
    '  $tmpG.Dispose(); $tmpBmp.Dispose()',
    '',
    '  $lh   = [float]($font.GetHeight($e.Graphics) + 2)',
    '  $bmpW = [int][Math]::Max($measW, 100)',
    ...
    '  }',
    '',
    '  # Escala o bitmap para caber exatamente na area imprimivel (evita corte lateral)',
    '  $scale   = $clipW / [float]$bmpW',
    '  $destH   = [float]($bmpH * $scale)',
    '  $destRect = New-Object System.Drawing.RectangleF([float]0, [float]0, $clipW, $destH)',
